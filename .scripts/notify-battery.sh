...
if [[ $# -eq 0 ]];then
	...
	notify-send -r 9977 -t 5000 "Battery Status" "$(cat /sys/class/power_supply/BAT1/capacity)% remaining" -i /usr/share/icons/Papirus/32x32/devices/battery.svg
	exit 1
fi
...
notify-send -r 9977 -t 5000 "USING ${pw} POWER" "capacity: ${capacity}%  remaining" -i $icon
